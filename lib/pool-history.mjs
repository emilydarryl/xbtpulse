...
export function capturePoolHistory(store,registry,research,now=Date.now(),gapMs=180000){
 ...
 const providers=profileProviderIds(store,registry,id).map(name=>({name,active:!!store.db.prepare('SELECT active FROM providers WHERE id=?').get(name)?.active,...store.db.prepare("SELECT MAX(end) AS lastReport, MAX(CASE WHEN json_extract(body,'$.work')>0 THEN end END) AS lastWorkReport FROM telemetry WHERE provider=?").get(name)}));
