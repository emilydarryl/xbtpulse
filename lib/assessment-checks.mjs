...
export function intervalMetrics(reports, start, end) {
  ...
  return {
    ...
    found: list.reduce((n, r) => n + r.found, 0),
...
export async function runAssessmentChecks(
  ...
  try {
    ...
    const summary = metrics
      .map(
        (p) =>
          `${p.id}: ${p.reports} complete intervals; ${p.coverage.toFixed(2)}% of requested time covered; ${p.gaps} uncovered ranges, largest ${p.largestGapMinutes} minutes; ${p.found} reported found / ${p.expected.toFixed(2)} expected. ${p.lastPositive ? "Last positive work " + new Date(p.lastPositive).toISOString() : "No positive work in this period"}.`,
      )
      .join("\n");
    const candidate =
      historyComplete &&
