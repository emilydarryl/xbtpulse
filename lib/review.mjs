...
export function reviewEvidence(store, application, now = Date.now()) {
  ...
  for (const p of providers) {
    ...
    checks.push({
      ...
      status: "Context only",
      detail: `${found} reported found / ${expected.toFixed(2)} expected in complete intervals. Mining luck varies; this ratio is not a pass/fail test. Chain attribution and matching time windows require review.`,
    });
  }
  return {
    status: checks
