...
export class Collector {
  ...
  async poll() {
    ...
    try {
      ...
      } else if (this.source.supportsCharacteristics && stored.slice(0,2016).some(b=>b.characteristicsVersion!==1)) {
        const high=stored.slice(0,2016).find(b=>b.characteristicsVersion!==1).height;
        fetched=await this.source.range(high,Math.max(stored.at(-1).height,high-this.batch+1));
      ...
      const combined = [...new Map([...stored, ...fetched].map(b=>[b.height,b])).values()]
