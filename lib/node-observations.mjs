...
export function parseNodeObservations(body, now = Date.now(), previous = null) {
  if (body?.schema !== 'btcb2.nodes/1' || !Array.isArray(body.nodes) || body.nodes.length > 10000) throw Error('Unknown node feed');
  let crawl = body.crawl;
  const resummary = crawl?.resummarised_only === true;
  if (resummary) {
    // A regenerated summary is not a new observation. Require an already recorded
    // matching crawl; never manufacture a scan window for a fresh installation.
    if (!previous || !integer(crawl.raw_mtime_epoch) ||
        Math.abs(crawl.raw_mtime_epoch * 1000 - previous.scanFinishedAt) > 60000 ||
        body.raw_records !== previous.candidatesAttempted || body.handshake_ok !== previous.handshakes ||
        body.nodes.length !== previous.reachable) throw Error('Resummary has no matching recorded scan');
    crawl = {started_at_epoch:previous.scanStartedAt/1000,finished_at_epoch:previous.scanFinishedAt/1000,
      partial:previous.partial,exit_code:0,max_nodes:previous.candidateLimitReached ? previous.candidatesAttempted : 0};
  }
  ...
  return {
    observedAt: resummary ? previous.observedAt : body.generated_at_epoch * 1000,
    summaryGeneratedAt: body.generated_at_epoch * 1000,
    resummarisedOnly: resummary,
  ...
  async function poll() {
    ...
    pending = (async () => {
      try {
        ...
        const previousRow = store.db.prepare('SELECT body FROM node_observations ORDER BY time DESC LIMIT 1').get();
        const previous = previousRow ? JSON.parse(previousRow.body) : null;
        const snapshot = parseNodeObservations(JSON.parse(raw+decoder.decode()), clock(), previous);
        const latest = previous?.observedAt ?? null;
        if (latest !== null && snapshot.observedAt < latest) throw Error('Regressed source timestamp');
        store.db.prepare('INSERT OR REPLACE INTO node_observations VALUES (?,?)').run(snapshot.observedAt, JSON.stringify(snapshot));
