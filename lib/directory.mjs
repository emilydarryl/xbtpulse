...
export function sortDirectory(rows, key = "name", direction = "asc") {
  const fields = {
    name: p => p.name,
    share: p => p.status?.observedShare,
    latest: p => p.status?.latestBlock?.height,
    hashrate: p => p.status?.reportedHashrate,
    networkShare: p => p.status?.reportedNetworkShare,
    tip: p => p.status?.reportedTip,
    protocols: p => p.status?.protocols,
  };
  const value = fields[key] || fields.name;
  const missing = v => v == null || v === "" || (typeof v === "number" && !Number.isFinite(v));
  return [...rows].sort((a,b) => {
    const av = value(a), bv = value(b);
    if (missing(av) !== missing(bv)) return missing(av) ? 1 : -1;
    const cmp = missing(av) ? 0 : typeof av === "number" ? av - bv : String(av).localeCompare(String(bv));
    return cmp * (direction === "desc" ? -1 : 1) || a.name.localeCompare(b.name) || a.id.localeCompare(b.id);
  });
}
