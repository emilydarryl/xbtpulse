...
async function load() {
  try {
    ...
    const expanded = new Set([...document.querySelectorAll('details[data-profile-detail][open]')].map(e=>e.dataset.profileDetail));
    ...
    document.querySelector('#pool-scores-summary').textContent=d.scorecard?'Published assessment':'Assessment · not assessed';
    if(firstProfileRender)document.querySelector('#pool-scores').open=!!d.scorecard;
    ...
    simplifyProfileDetails(expanded);
...
function simplifyProfileDetails(expanded){
 const root=document.querySelector('#content');
 const evidence=document.querySelector('#public-evidence section');
 for(const [title,key] of [['Block production across windows','windows'],['Block characteristics','characteristics'],['Recent attributed blocks','recent'],['Coinbase payout recipients','recipients']]){
  const heading=[...root.querySelectorAll('h2')].find(e=>e.textContent===title);
  const section=heading?.closest('section');if(!section)continue;
  const detail=document.createElement('details');detail.dataset.profileDetail=key;detail.open=expanded.has(key);
  const summary=document.createElement('summary');summary.textContent=title;detail.append(summary);
  section.classList.remove('panel','intake-panel');detail.append(section);evidence.append(detail);
 }
 for(const detail of document.querySelectorAll('#public-evidence details[data-profile-detail]'))detail.open=expanded.has(detail.dataset.profileDetail);
}
