...
Shared site navigation: Dashboard, Pools, Tools, My watchlist and Contribute appear in every served HTML page header. Tools groups Compare pools, Mining simulator, Who builds my block?, Trends & changes and Mining map. Narrow screens use a Menu button. The homepage chart tabs contain only Network overview, Payout addresses, Template control and Methodology, wrapping when needed. Dashboard is the explicit route back to the main site. Existing sample/admin header controls are retained.

The homepage Pool distribution table has a bounded vertical scroll area (approximately 8–10 desktop rows) with sticky column headings. Focus the area to scroll using the keyboard. **View all pools** opens the full searchable directory. Other tables retain their existing behavior.

Click a recipient in Payout-address activity to inspect its full address/script, selected-window versus retained-history totals, first/last appearances, documented roles with sources, attributed groups of paying blocks, and the latest 20 matching blocks with tags and hashes. Copy the full recipient using the button. Retained history is not all-time history, receipts are not a wallet balance, and associated block labels do not identify the recipient owner. Sample mode remains illustrative.
