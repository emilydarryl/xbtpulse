...
- **Crypto-Eire:** modified RATUM Prime v0.1.22; public stats endpoint and compatibility findings are documented in `docs/crypto-eire-compatibility.md`. Compatibility is still evidence-driven; do not assume a hashrate/worker endpoint provides accepted-work counters. Historical fee clarification distinguished hosted 3%, own gateway 1% pool fee, and solo 5%; treat those as dated operator claims, not current verified universal terms. Consult current published sources before updating them.
