...
Validation before deployment: 102 Node tests, syntax and collector packaging checks passed; focused observer tests passed after the final history query adjustment. Real-browser local checks confirmed the observed 187-endpoint snapshot, five release/RC groups, source/cap labels and responsive table. Deployed application revision: `a883d7159ecd14f0c0c01b5517d82b1016dcede1`, recorded in `/opt/xbtpulse/DEPLOYED_COMMIT`. Live `/healthz` and `/readyz` passed; `/api/nodes` returned current status and the September 21 14:12:50 UTC source snapshot (187 endpoints, 114 IPv4, 73 Tor, 0 IPv6, five advertised release/build groups). Live browser confirmed source attribution, version breakdowns, full-string disclosure, history-building and candidate-cap labels. These counts are dated observations, not permanent network facts. Existing untracked `tmp/` was preserved.
...
## Homepage operator contributions placement
...
## Private coinbase tag search (September 17)
