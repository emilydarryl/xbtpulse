...
`config/pools.json` contains reviewed rules and provider mappings; a provider mapping alone does not attribute blocks. Explorer pool names are shown as `Explorer attribution`, with source metadata retained on each block. Do not silently describe these as independently verified pools.
...
## Reviewed collection-address rule

See the [AlphaPool reproduction and scope](alphapool-attribution.md). Address rules with `soleRecipient: true` match only one positive-value output; a zero-value output cannot attribute a block. No CEO/Test tag aliases are matched without the address evidence. Historical charts are recalculated using the current registry.
