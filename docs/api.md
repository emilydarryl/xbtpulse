...
Pool responses also contain `blockReports`: optional operator-reported individual finds with separate chain-match status, a newest-50 list, total count, 90-day retention and source freshness. These reports do not change attributed block totals or telemetry outcomes. See [the block-report feed contract](block-reporting.md) for fields, chain states, authenticated submission and withdrawal.
